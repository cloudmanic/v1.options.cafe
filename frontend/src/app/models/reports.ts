...
export class AccountReturn {
	...
	// Build from json list.
	//
	fromJsonList(json: Object[]): AccountReturn[] {
		let list: AccountReturn[] = [];
	...
	// Json to Object.
	//
	fromJson(json: Object): AccountReturn {
		let obj = new AccountReturn();

		obj.Date = moment(json["date"]).toDate();
		obj.Percent = json["percent"];
		obj.AccountValue = json["account_value"];
		obj.TotalCash = json["total_cash"];
		obj.PricePer = json["price_per"];
		obj.Units = json["units"];
