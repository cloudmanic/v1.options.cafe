...
import { SummaryYearly, ProfitLoss, AccountReturn } from '../../models/reports';
...
export class ReportsService {
	...
	//
	getAccountReturns(brokerAccount: number, start: Date, end: Date): Observable<AccountReturn[]> {
		let startDate = moment(start).format('YYYY-MM-DD');
		let endDate = moment(end).format('YYYY-MM-DD');
		let getParms = "start=" + startDate + "&end=" + endDate;

		return this.http.get<AccountReturn[]>(environment.app_server + '/api/v1/reports/' + brokerAccount + '/account-returns?' + getParms)
			.map((data) => { return new AccountReturn().fromJsonList(data); });
	}
}
