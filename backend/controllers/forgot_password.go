...
func DoResetPassword(w http.ResponseWriter, r *http.Request) {
  ...
  user, err := DB.GetUserFromToken(post.Hash)
  ...
  err = DB.ValidatePassword(post.Password)  
  ...
  err = DB.ResetUserPassword(user.Id, post.Password)
  ...
  err = DB.DeleteForgotPasswordByToken(post.Hash)
