...
//
func (t *DB) GetPositionByUserSymbolStatusAccount(userId uint, symbolId uint, status string, brokerAccountId uint) (Position, error) {

	var position = Position{}

	// First we find out if we already have a position on for this.
	if t.Where("symbol_id = ? AND user_id = ? AND status = ? AND broker_account_id = ?", symbolId, userId, status, brokerAccountId).First(&position).RecordNotFound() {
