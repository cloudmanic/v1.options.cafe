...
// 	screenerResults, err := bt.LongCallButterflySpreadResults(db, today, &backtest, underlyingLast, options, cache)
// 	st.Expect(t, err, nil)
...
// 	screenerResults2, err := bt2.LongCallButterflySpreadResults(db, tomorrow, &backtest, underlyingLast, options, cache)
// 	st.Expect(t, err, nil)
// 	st.Expect(t, len(screenerResults), 4)
// 	st.Expect(t, screenerResults[0].Debit, 2.84)
// 	st.Expect(t, screenerResults[0].MidPoint, 0.27)
...
// 	// Test the function we are here to test.
// 	bt.LongCallButterflySpreadPlaceTrades(db, today, &backtest, screenerResults, options, cache)
// 	st.Expect(t, len(backtest.TradeGroups), 1)

// 	// Test the function we are here to test. (nothing should change because we already have a trade on.)
// 	bt2.LongCallButterflySpreadPlaceTrades(db, tomorrow, &backtest, screenerResults2, options, cache)
// 	st.Expect(t, len(backtest.TradeGroups), 1)
